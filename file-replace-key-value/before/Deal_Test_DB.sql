...
-- 1、先检查GLOBAL_ORGAN内UNION_CODE的去重数量；
select count(distinct (UNION_CODE)) from GLOBAL_ORGAN;
select count(UNION_CODE) from GLOBAL_ORGAN;
select count(distinct(NAME)) from GLOBAL_ORGAN;
select count(NAME) from GLOBAL_ORGAN;

-- 2、机构信息289条导入到GLOBAL_ORGAN体系；
-- 执行20250517不在3030内企业-处理.sql
-- update GLOBAL_ORGAN set del_flag=0, CREATE_TYPE=0, create_time = now(), LAST_TIME=now() where SPARE1 = '不在3030内的企业，新增的289企业';
-- update GLOBAL_ORGAN set create_time = now() where create_time is null;
-- insert into GLOBAL_ORGAN_EXT (ORGAN_ID)
-- select ORGAN_ID from GLOBAL_ORGAN where ORGAN_ID not in (select ORGAN_ID from GLOBAL_ORGAN_EXT);
-- insert into SYS_ORGAN (SYSORGAN_ID, DEL_FLAG, CREATE_TIME, LAST_TIME)
-- select ORGAN_ID, 0, now(), now() from GLOBAL_ORGAN where ORGAN_ID not in (select SYSORGAN_ID from SYS_ORGAN);
-- insert into SYS_ORGAN_EXT (SYSORGAN_ID)
-- select ORGAN_ID from GLOBAL_ORGAN where ORGAN_ID not in (select SYSORGAN_ID from SYS_ORGAN_EXT);
select * from SAS_PEOPLE_BASE where REGISTER_ORGAN_NATURE not in (select UNION_CODE from GLOBAL_ORGAN) AND PEOPLE_TYPE='PEOPLE_INFO';

-- 3、检查社保接口执行后，SAS_PEOPLE_BASE内ORGAN_NAME和REGISTER_ORGAN_NAME不一致的数据，分析后确定是否有新增的企业，是否将REGISTER_NATURE（统一信用代码）更新ORGAN_ID和ORGAN_NAME；
select * from SAS_PEOPLE_BASE where ORGAN_NAME <> REGISTER_ORGAN_NAME AND PEOPLE_TYPE='PEOPLE_INFO';
select distinct (REGISTER_ORGAN_NAME) from SAS_PEOPLE_BASE where ORGAN_NAME <> REGISTER_ORGAN_NAME;
select REGISTER_ORGAN_NAME, REGISTER_ORGAN_NATURE, '0518社保接口内增加的企业' from SAS_PEOPLE_BASE
where REGISTER_ORGAN_NATURE not in (select UNION_CODE from GLOBAL_ORGAN) AND PEOPLE_TYPE='PEOPLE_INFO';

-- 4、将新增的企业加入GLOBAL_ORGAN体系；用模板《重点扶持企业分类认定名单-导入》导入社保接口后新增的X条新企业
-- insert into GLOBAL_ORGAN (ORGAN_ID, NAME, UNION_CODE, SPARE1, DEL_FLGA, CREATE_TYPE, CREATE_TIME, LAST_TIME)
-- select guid(), REGISTER_ORGAN_NAME, REGISTER_ORGAN_NATURE, '0518社保接口内增加的企业', 0, 0, now(), now() from SAS_PEOPLE_BASE
-- where REGISTER_ORGAN_NATURE not in (select UNION_CODE from GLOBAL_ORGAN) AND PEOPLE_TYPE='PEOPLE_INFO';
-- insert into GLOBAL_ORGAN_EXT (ORGAN_ID)
-- select ORGAN_ID from GLOBAL_ORGAN where ORGAN_ID not in (select ORGAN_ID from GLOBAL_ORGAN_EXT);
-- insert into SYS_ORGAN (SYSORGAN_ID, DEL_FLAG, CREATE_TIME, LAST_TIME)
...
-- 5、更新SAS_ORGAN_BASE基于社保变化的企业；
select * from GLOBAL_ORGAN GLO where ORGAN_ID not in (select ORGAN_ID from SAS_ORGAN_BASE ) and LENGTH(UNION_CODE)>16;
INSERT INTO SAS_ORGAN_BASE (ORGAN_ID, ORGAN_NAME, UNION_CODE, ORGAN_LEVEL, CREATE_TIME, CONN_GORGAN_ID, DEL_FLAG, SPARE1)
select GLO.ORGAN_ID, GLO.NAME, GLO.UNION_CODE, 'GRC', GLO.CREATE_TIME, GLO.ORGAN_ID, 0, SPARE1 from GLOBAL_ORGAN GLO where ORGAN_ID not in (select ORGAN_ID from SAS_ORGAN_BASE ) and LENGTH(UNION_CODE)>16;
update SAS_ORGAN_BASE set DEL_FLAG = 0 where DEL_FLAG IS NULL;
-- 6、将新增的企业插入到GRC_ORGAN_RENCAI中；
-- select * from GLOBAL_ORGAN GLO where ORGAN_ID not in (select ORGAN_ID from GRC_ORGAN_RENCAI ) and LENGTH(UNION_CODE)>16;
-- INSERT INTO GRC_ORGAN_RENCAI (ORGAN_ID, ORGAN_NAME, STATUS, CREATOR_ID, CREATE_TIME, SPARE1)
-- select GLO.ORGAN_ID, GLO.NAME, 'WAIT_RECEIVE', '0518COMPARE', GLO.CREATE_TIME, SPARE1 from GLOBAL_ORGAN GLO where ORGAN_ID not in (select ORGAN_ID from SAS_ORGAN_BASE ) and LENGTH(UNION_CODE)>16;

-- 7、检查已有企业ORGAN_ID是否保持一致，并更新SAS_ORGAN_BASE内ORGAN_ID和CONN_GORGAN_ID一致；
select * from SAS_ORGAN_BASE where CONN_GORGAN_ID <> ORGAN_ID;
update SAS_ORGAN_BASE set CONN_GORGAN_ID = ORGAN_ID ;
--
-- -- 人员信息处理
-- 1、导入白名单数据和灰名单数据，分析白名单内PEOPLE_ID为空的数据，是否需要增加人员到SAS_PEOPLE_BASE和GLOBAL_USER体系；
select * from GRC_ORGAN_USER_FMLIST where SPARE1 not in (select UNION_CODE from GLOBAL_ORGAN);
...
-- 2、将新增的用户添加到GLOBAL_USER体系中；
-- 没有手机号，不添加账户
-- 3、将新增的用户添加到SAS_PEOPLE_BASE中；
...
select SPARE2, B.NAME, B.ID_NUM, B.TELEPHONE, FOREIGN_ID, SERVER_ORGAN_ID, SERVER_ORGAN_NAME, SPARE1, 'PEOPLE_INFO' from GRC_ORGAN_USER_FMLIST B where PEOPLE_USER_ID IS NULL and ID_NUM NOT IN (select ID_NUM from SAS_PEOPLE_BASE);
-- 4、将新增的人员添加到GRC_PEOPLE_RENCAI中；

-- 5、将新增的人员添加到GRC_ORGAN_USER_LIST中，类型为EMPLOYEE；

-- -- 已有人员和机构关联信息机构为空处理
-- 1、更新当前SAS_PEOPLE_BASE表中ORGAN_ID为空的数据，更新ORGAN_NAME；
select * from SAS_PEOPLE_BASE where ORGAN_ID IS NULL;
select * from SAS_PEOPLE_BASE SPB, GLOBAL_ORGAN GLO
where SPB.REGISTER_ORGAN_NATURE = GLO.UNION_CODE AND SPB.ORGAN_ID IS NULL and REGISTER_ORGAN_NATURE IS NOT NULL;
UPDATE SAS_PEOPLE_BASE SPB
set SPB.ORGAN_ID= GLO.ORGAN_ID, SPB.ORGAN_NAME=GLO.NAME
    FROM SAS_PEOPLE_BASE SPB, GLOBAL_ORGAN GLO
...
-- 2、更新当前SAS_USER_RANGE_ORGAN表中ORGAN_ID为空的数据；
select SURO.ORGAN_ID, SPB.ORGAN_ID, SPB.ID_NAME from SAS_USER_RANGE_ORGAN SURO, SAS_PEOPLE_BASE SPB
WHERE SURO.USER_ID = SPB.PEOPLE_ID AND PEOPLE_TYPE='PEOPLE_INFO';
update SAS_USER_RANGE_ORGAN SURO
set SURO.ORGAN_ID = SPB.ORGAN_ID
    from SAS_USER_RANGE_ORGAN SURO, SAS_PEOPLE_BASE SPB
...
SELECT * from GRC_PEOPLE_RENCAI GPR, SAS_PEOPLE_BASE SPB
...
set GPR.CURR_ORGAN_ID = SPB.ORGAN_ID, GPR.CURR_ORGAN_NAME=SPB.ORGAN_NAME
    from GRC_PEOPLE_RENCAI GPR, SAS_PEOPLE_BASE SPB
where GPR.PEOPLE_ID = SPB.PEOPLE_ID and GPR.CURR_ORGAN_ID IS NULL and SPB.ORGAN_ID IS NOT NULL AND PEOPLE_TYPE='PEOPLE_INFO';
-- 4、更新当前GRC_ORGAN_USER_LIST表中ORGAN_ID为空的数据；
SELECT * from GRC_ORGAN_USER_LIST GOUL , SAS_PEOPLE_BASE SPB
...
set GOUL.SERVER_ORGAN_ID = SPB.ORGAN_ID, GOUL.SERVER_ORGAN_NAME=SPB.ORGAN_NAME
    from GRC_ORGAN_USER_LIST GOUL, SAS_PEOPLE_BASE SPB
where GOUL.PEOPLE_USER_ID = SPB.PEOPLE_ID and GOUL.SERVER_ORGAN_ID IS NULL and SPB.ORGAN_ID IS NOT NULL AND SPB.PEOPLE_TYPE='PEOPLE_INFO';
-- 5、更新GRC_PEOPLE_POST内ORGAN_ID、ORGAN_NAME、UNION_CODE；
SELECT * from GRC_PEOPLE_POST GPP , SAS_PEOPLE_BASE SPB
...
set GPP.ORGAN_ID = SPB.ORGAN_ID, GPP.ORGAN_NAME=SPB.ORGAN_NAME, GPP.UNION_CODE=SPB.REGISTER_ORGAN_NATURE
    from GRC_PEOPLE_POST GPP, SAS_PEOPLE_BASE SPB
where GPP.PEOPLE_ID = SPB.PEOPLE_ID and GPP.ORGAN_ID IS NULL and SPB.ORGAN_ID IS NOT NULL AND PEOPLE_TYPE='PEOPLE_INFO';
-- 6、查看当前SAS_USER_IDENTITY表中ORGAN_ID为空的数据；
select * from SAS_USER_IDENTITY where ORGAN_ID IS NULL;
...
-- 7、检查银行卡信息是否正常；
select * from GRC_PEOPLE_BANKINFO GPB, SAS_PEOPLE_BASE SPB
...
set GPB.PEOPLE_ID = SPB.PEOPLE_ID
    from GRC_PEOPLE_BANKINFO GPB, SAS_PEOPLE_BASE SPB
where GPB.PEOPLE_ID IS NULL and SPB.ID_NUM = ( SELECT HEX(AES_ENCRYPT(CARD_OWNER, 'BmSHFCFwF0No5z91YwCL653QqEGrwiCd4s8Pnp5v4pzfgZO/ST6F0IucsT+MO4iOGHI9PMhhazeGHhrYD65QXHdxd8/jVlq3I9GauHKzWyKvL3rY3/c/T/2D2fmgAmieudSZuwKtgxb1O+9ypC9+iUH2uzl0s60qFaSNYMZLxmPfr3OPeQZpmSZmimq/QDBZryQ02E1vKkY=')) )
-- 8、检查各类字典是否正常，重点是前置审批的字典；
--
-- -- 验证用户登录后状态是否正常
-- 1、测试配置了GLOBAL_USER用户体系的用户是否能够正常登录，登录后，SAS_USER_IDENTITY表中ORGAN_ID为空的数据是否更新了ORGAN_ID，以及再次登录是否正常；
...
delete from IOC_MANAGE_2_1_2 where MEMBMER_ID not in (select MEMBER_ID From VIP_MEMBER_USER);
delete from IOC_MANAGE_2_2_1 where USER_ID not in (select MEMBER_ID From VIP_MEMBER_USER);
-- 删除support层账户
...
--      测试库删除多余添加的用户
-- 删除system层账户
delete from GLOBAL_USER where USER_ID not in ALLOW_USER_LIST;
delete from GLOBAL_USER_EXT where USER_ID not in ALLOW_USER_LIST;
delete from GLOBAL_USER_PROPERTY where USER_ID not in ALLOW_USER_LIST;
delete from SYS_USER where USER_ID not in ALLOW_USER_LIST;
delete from SYS_USER_EXT where USER_ID not in ALLOW_USER_LIST;

-- 删除manage层账户
delete from VIP_MEMBER_USER where USER_ID not in ALLOW_USER_LIST;
delete from VIP_MEMBER_TOKEN where MEMBMER_ID not in (select MEMBER_ID From VIP_MEMBER_USER);
delete from VIP_THIRD_ACCOUNT_USER where USER_ID not in (select MEMBER_ID From VIP_MEMBER_USER);
-- 删除support层账户
delete from SAS_USER_IDENTITY where USER_ID not in ALLOW_USER_LIST;
delete from SAS_USER_RANGE_ORGAN where USER_ID not in ALLOW_USER_LIST;
delete from SAS_USER_PERSONAL where USER_ID not in ALLOW_USER_LIST;
...
-- 基础库
delete from SAS_PEOPLE_BASE where PEOPLE_ID not in ALLOW_USER_LIST;
